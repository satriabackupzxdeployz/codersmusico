...
function buildShareUrl(track: PipedTrack) {
  ...
  return `${window.location.origin}/track/${track.videoId}-${slug}`;
}
    ...
    <div ref={forwardRef} className="canvas-export-root">
      <div className="player-phone-shell canvas-export-shell">
        <div className="player-card-box">
          <div className="player-card-surface">
            <div className="player-art-container-ref">
              <img src={track.artwork} alt={track.title} crossOrigin="anonymous" />
            </div>
  ...
  const generateCanvasImage = async () => {
    ...
    try {
      ...
      const loaded = new Promise<void>((resolve, reject) => {
        ...
        image.onerror = () => reject(new Error("Gagal merender canvas."));
      });
      image.src = svgUrl;
      await loaded;
            ...
            return (
              <>
                ...
                <div className="artist-actions-row">
                  ...
                  <button type="button" className="artist-play-btn" onClick={() => tracks[0] && openTrack(tracks[0], tracks, playlist?.name || "Playlist")}>
                    <PlayIcon />
                  </button>
                </div>
                  ...
                  <div className="vertical-list">
                    {tracks.map((track) => (
                      <div key={track.id} className="v-item playlist-track-row">
                        <button type="button" className="track-cover-btn" onClick={() => openTrack(track, tracks, playlist?.name || "Playlist")}>
                          <img className="v-img" src={track.artwork} alt={track.title} />
                        </button>
                        <button type="button" className="v-info text-left track-title-btn" onClick={() => openTrack(track, tracks, playlist?.name || "Playlist")}>
                          <div className="v-title">{track.title}</div>
                          <div className="v-sub">{track.artist}</div>
                        </button>
                        <button
          ...
          <div className="player-modal-primary">
            ...
            {playerMenuOpen && currentTrack && (
              <div className="player-menu-sheet overlay">
                ...
                <button type="button" className="player-menu-item" onClick={() => openCanvasSheet("box")}>
                  Buat Gambar (Music Box)
                </button>
                <button type="button" className="player-menu-item" onClick={() => openCanvasSheet("lyrics")}>
                  Buat Gambar (with Lyrics)
                </button>
